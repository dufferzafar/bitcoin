...
// This was called when we were sure that it extends on our Tip
// What are the other cases it could be called??
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    // Someone else has sent me an inventory message
    // containing the list of objects that they have?
    ...
    else if (strCommand == NetMsgType::ANCHOR)
    {
        ...
        LogPrint(BCLog::NET, "received anchor %s from peer=%d\n", panchor->GetHash().ToString(), pfrom->GetId());

        // Check if Anchor is duplicate
        if (mapAnchors.count(panchor->GetHash())) {
            LogPrint(BCLog::NET, "anchor is duplicate %s from peer=%d\n", panchor->GetHash().ToString(), pfrom->GetId());
        } else {
